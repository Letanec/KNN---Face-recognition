...
def get_datasets(batch_size, batch_size_test = 128):
    ...
    casia_pairs_dataloader = DataLoader(dataset, batch_size=batch_size_test, shuffle=True)
    #LFW DATASET
    dataset = CustomDataset(csv_file = 'datasets/LFW/lfw_pairs.csv', root_dir = 'datasets/LFW', transform = transform, pairs=True) 
    lfw_pairs_dataloader = DataLoader(dataset, batch_size=batch_size_test, shuffle=True)
